...
def main():
    ...
    # Re-sort contributors by verified_tasks descending, then combinations
    contributors = leaderboard.get("contributors", [])
    contributors.sort(key=lambda c: (c.get("verified_tasks", 0), c.get("combinations", 0)), reverse=True)
    ...
    if total_combos > stats.get("total_combinations", 0):
    ...
    leaderboard["total_combinations"] = stats["total_combinations"]
    ...
    print(f"24h Aggregation Complete: {total_tasks} verified tasks, {stats['total_combinations']:,} total combinations.")
